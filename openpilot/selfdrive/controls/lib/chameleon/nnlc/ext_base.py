...
sunnypilot/selfdrive/controls/lib/latcontrol_torque_ext_base.py (twilsonco's
lateral-jerk lookahead machinery). Changes: the constructor takes
(lac_torque, CP) — no CP_SP, and the torque-space conversion comes from this
package's torque_space module instead of a CI method, since chameleonpilot
pins commaai's opendbc. NNLC runs its own PID with KP=1.0/KI=0.3 — hotter
than upstream's KP=0.8/KI=0.15 — with limits in TORQUE space (±steer_max),
where upstream's PID limits live in lateral-accel space.
"""
import math
import numpy as np
...
from openpilot.selfdrive.controls.lib.chameleon.nnlc.torque_space import torque_from_lateral_accel_in_torque_space
...
class LatControlTorqueExtBase:
  def __init__(self, lac_torque, CP):
    ...
    self.torque_from_lateral_accel_in_torque_space = torque_from_lateral_accel_in_torque_space
